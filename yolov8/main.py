...
cap = cv2.VideoCapture('../../../../data/peoplecount1.mp4')
...
while True:
    ...
    for bbox in bbox_id:
        x3, y3, x4, y4, id = bbox
        #---------------- people entering -----------------
        results = cv2.pointPolygonTest(np.array(area2, np.int32), ((x4, y4)), False) #print(results) # point(x2, y2) area2 nin içerisindeyse results = 1 değilse results = -1
        if results >= 0:
